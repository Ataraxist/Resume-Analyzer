...
function PreviousResumes({ onSelectResume, onRefresh }) {
  ...
  const fetchMyResumes = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await resumeService.getMyResumes(10);
      if (response.success) {
  ...
  const handleSelectResume = async (resume) => {
    try {
      ...
      const structuredData = await resumeService.getStructuredData(resume.id);
  ...
  const formatDate = (dateString) => {
    ...
    const utcDateString = dateString.includes('Z') || dateString.includes('+') || dateString.includes('-') 
      ? dateString 
